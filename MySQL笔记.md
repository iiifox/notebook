...
```ini
default-character-set=utf8 #默认字符集 [mysql]
...
2: 通过命令行+特殊参数开启mysqld `mysqld -- defaults-file="D:\MySQL\MySQLServer5.7\data\my.ini" --skip-grant-tables `
...
### 问题3：图形化界面连接MySQL8.0时加密规则问题

解决方案一：升级图形化界面工具

解决方案二：加密规则由MySQL8.0新增的 `caching_sha2_password` 还原为 5.7 时的 `mysql_native_password`
...
*************************** 1. row ***************************
Table: student
Create Table: CREATE TABLE `student` ( `id` int(11) NOT NULL, `name` varchar(20) CHARACTER SET latin1 DEFAULT NULL, PRIMARY KEY (`id`) ) ENGINE=InnoDB DEFAULT CHARSET=utf8  #字段仍然是latin1编码
...
2. `DISTINCT`其实是**对后面所有列名的组合进行去重**。如果你想要看都有哪些不同的部门（department_id），只需要写`DISTINCT department_id`即可，后面不需要再加其他的列名了。
...
（2）‘$’匹配以该字符前面的字符结尾的字符串。
（3）‘.’匹配任何一个单字符。
（4）“[...]”匹配在方括号内的任何字符。例如，“[abc]”匹配“a”或“b”或“c”。为了命名字符的范围，使用一 个‘-’。“[a-z]”匹配任何小写字母，而“[0-9]”匹配任何数字。
...
- A∪B
  ...
  ```mysql
  # 实现查询结果是A∪B
  # 用左外的A，union 右外的B。从效率上来说，应采用 union all
  select 字段列表
  from A表 left join B表
  on 关联条件
  where 等其他子句
...
FROM employees e JOIN departments d
ON e.department_id = d.department_id;
# USAING :指定数据表里的同名字段进行等值连接，只能配合JOIN一起使用。
SELECT employee_id,last_name,department_name
FROM employees e JOIN departments d
USING (department_id);
```
...
3. 添加外部行。如果我们使用的是左外连接、右外连接或者全外连接，就会涉及到外部行，也就是在虚拟表 vt1-2 的基础上增加外部行，得到虚拟表 vt1-3。
...
![MySQL服务器端的逻辑架构说明](MySQL笔记.assets/202203291130782.png)
...
![image-20220329113126234](MySQL笔记.assets/202203291131321.png)
...
![image-20220329113337008](MySQL笔记.assets/202203291133116.png)
...
![image-20220329114541991](MySQL笔记.assets/202203291145074.png)
...
![image-20220329115517901](MySQL笔记.assets/202203291155004.png)
...
![image-20220329115958009](MySQL笔记.assets/202203291159111.png)
...
![image-20220330095642965](MySQL笔记.assets/202203300956080.png)
...
![image-20220330100053271](MySQL笔记.assets/202203301000365.png)

把一些记录放到页里的示意图就是：

![image-20220330100150297](MySQL笔记.assets/202203301001377.png)
...
![image-20220330100409306](MySQL笔记.assets/202203301004386.png)
...
![image-20220330102834658](MySQL笔记.assets/202203301028743.png)
...
![image-20220330103442914](MySQL笔记.assets/202203301034997.png)
...
![image-20220330105018965](MySQL笔记.assets/202203301050055.png)
...
![image-20220330111029120](MySQL笔记.assets/202203301110222.png)
...
![image-20220330112018199](MySQL笔记.assets/202203301120304.png)
...
![image-20220330113307813](MySQL笔记.assets/202203301133907.png)
...
![image-20220330163602743](MySQL笔记.assets/202203301636830.png)
...
![image-20220330163506536](MySQL笔记.assets/202203301635653.png)

针对同样的数据，如果我们把二叉树改成`M 叉树`（M>2）呢？当 M=3 时，同样的 31 个节点可以由下面的三叉树来进行存储：

![image-20220330163709022](MySQL笔记.assets/202203301637104.png)

##### **5.3 B-Tree** 

B 树的结构如下图所示：

![image-20220330163803905](MySQL笔记.assets/202203301638995.png)
...
![image-20220330164411665](MySQL笔记.assets/202203301644761.png)
...
![image-20220330183814954](MySQL笔记.assets/202203301838071.png)
...
![image-20220330191335574](MySQL笔记.assets/202203301913664.png)

这两条记录`不是我们自己定义的记录`，所以它们并不存放在页的User Records部分，他们被单独放在一个称为Infimum + Supremum的部分

![image-20220330191446070](MySQL笔记.assets/202203301914167.png)
...
![image-20220330192130497](MySQL笔记.assets/202203301921631.png)
...
![image-20220330192413776](MySQL笔记.assets/202203301924874.png)
...
![image-20220330193949517](MySQL笔记.assets/202203301939611.png)
...
![image-20220330194534127](MySQL笔记.assets/202203301945235.png)
...
![image-20220401182649509](MySQL笔记.assets/202204011826671.png)
...
![image-20220401183344880](MySQL笔记.assets/202204011833000.png)
...
![image-20220403092826169](MySQL笔记.assets/202204030928295.png)
...
![image-20220403102536170](MySQL笔记.assets/202204031025279.png)

**2、数据分片**

![image-20220403102618627](MySQL笔记.assets/202204031026743.png)
...
![image-20220403110448951](MySQL笔记.assets/202204031104063.png)
...
![image-20220403112416944](MySQL笔记.assets/202204031124086.png)

**2.** **脏读（**`Dirty Read`**）**

对于两个事务 Session A、Session B，Session A`读取`了已经被 Session B`更新`但还`没有被提交`的字段。之后若 Session B`回滚`，Session A`读取`的内容就是`临时且无效`的。

![image-20220403112435995](MySQL笔记.assets/202204031124136.png)

**3.** **不可重复读（**`Non-Repeatable Read`**）**

对于两个事务Session A、Session B，Session A`读取`了一个字段，然后 Session B`更新`了该字段。 之后Session A`再次读取`同一个字段，`值就不同`了。那就意味着发生了不可重复读。

![image-20220403112458183](MySQL笔记.assets/202204031124331.png)

**4.** **幻读（**`Phantom`**）**

对于两个事务Session A、Session B, Session A 从一个表中`读取`了一个字段, 然后 Session B 在该表中`插入`了一些新的行。 之后, 如果 Session A`再次读取`同一个表, 就会多出几行。那就意味着发生了幻读。

![image-20220403112514712](MySQL笔记.assets/202204031125847.png)
...
![image-20220403112740425](MySQL笔记.assets/202204031127542.png)
...
![image-20220403114709581](MySQL笔记.assets/202204031147714.png)
...
![image-20220403115232833](MySQL笔记.assets/202204031152952.png)

![image-20220403115249492](MySQL笔记.assets/202204031152607.png)

![image-20220403115300809](MySQL笔记.assets/202204031153931.png)
...
![image-20220404091224993](MySQL笔记.assets/202204040912119.png)

**2. redo** **日志写入log buffer**

不同的事务可能是`并发`执行的，所以`事务T1`、`事务T2`之间的`mtr`可能是`交替执行`的。

![image-20220404091511602](MySQL笔记.assets/202204040915708.png)
...
![image-20220404092038421](MySQL笔记.assets/202204040920532-165020554065758.png)

**3. checkpoint**

![image-20220404092106617](MySQL笔记.assets/202204040921715.png)
...
![image-20220404093706650](MySQL笔记.assets/202204040937761.png)

**有了Redo Log和Undo Log之后：**

![image-20220404093832512](MySQL笔记.assets/202204040938657.png)

**2.** **详细生成过程**

![image-20220404093950136](MySQL笔记.assets/202204040939231.png)
...
![image-20220404094033441](MySQL笔记.assets/202204040940543.png)

**当我们执行UPDATE时：**

![image-20220404094105525](MySQL笔记.assets/202204040941624.png)

```mysql
UPDATE user SET id=2 WHERE id=1;
```

![image-20220404094142337](MySQL笔记.assets/202204040941449.png)
...
![image-20220404094436830](MySQL笔记.assets/202204040944948.png)
...
![image-20220405093322184](MySQL笔记.assets/202204050933339.png)
...
![image-20220405151409557](MySQL笔记.assets/202204051514736.png)
...
![image-20220405151829547](MySQL笔记.assets/202204051518659.png)
...
![image-20220405153617536](MySQL笔记.assets/202204051536648.png)

另图：

![image-20220405153632021](MySQL笔记.assets/202204051536125.png)
...
![image-20220405155041964](MySQL笔记.assets/202204051550072.png)
...
![image-20220405155640520](MySQL笔记.assets/202204051556631.png)
...
![image-20220405155909223](MySQL笔记.assets/202204051559345.png)
...
![image-20220405155941753](MySQL笔记.assets/202204051559867.png)
...
![image-20220405163025361](MySQL笔记.assets/202204051630535.png)
...
![image-20220405163125180](MySQL笔记.assets/202204051631346.png)

为了安全起见，可以设置为`1`，表示每次提交事务都会执行fsync，就如同**redo log** **刷盘流程**一样。最后还有一种折中方式，可以设置为N(N>1)，表示每次提交事务都write，但累积N个事务后才fsync。

![image-20220405163205364](MySQL笔记.assets/202204051632526.png)
...
![image-20220405163716222](MySQL笔记.assets/202204051637390.png)

使用**两阶段提交**后，写入binlog时发生异常也不会有影响

![image-20220405163902977](MySQL笔记.assets/202204051639192.png)

另一个场景，redo log设置commit阶段发生异常，那会不会回滚事务呢？

![image-20220405163927129](MySQL笔记.assets/202204051639403.png)
...
![image-20220405164559961](MySQL笔记.assets/202204051646097.png)
...
![image-20220405164718627](MySQL笔记.assets/202204051647759.png)
...
![image-20220405165455998](MySQL笔记.assets/202204051654133.png)

**方法** **2：半同步复制**

![image-20220405165513025](MySQL笔记.assets/202204051655175.png)
...
![image-20220405165650425](MySQL笔记.assets/202204051656560.png)
